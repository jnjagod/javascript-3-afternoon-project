...
//Code Here
let evenNumbers = mixedNumbers.filter(function(elem){return elem % 2 ===0})
...
//Code Here
let postTaxPrices = prices.map((elem) => {return elem * 1.07})
...
//Code Here
let totalPopulation = populations.reduce((acc, elem) => {return acc + elem})
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(elem){return elem["CP"] > 200})
...
let orderTotals = orders.map(function(elem){return elem["price"] * (1 + elem["tax"])})
...
let bobsTotal = purchases.filter((elem) => {return elem["owner"] === "Bob"}).map((elem) => {return elem["price"]}).reduce((acc, elem) => {return acc + elem})
